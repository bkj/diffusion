...
#For some reason this is not working for X_train/y_train
...
for prop in props:
    ...
    #RBF (indexing only kmedioids train points may be incorrect?)
    model2 = svm.SVC(gamma='scale', decision_function_shape='ovo', kernel='rbf').fit(X[train,:], y[train])
    pred2 = model2.predict(X_test)
    acc_rbf = metric_fn(y_test, pred2)
    print('SVC Kernel', acc_rbf)

    results.append([prop, acc_weighted, acc_unweighted, acc_kmedioid_k, acc_kmedioid_kplus, acc_svc, acc_rbf])

df = pd.DataFrame(results, columns=['prop', 'acc_weighted', 'acc_unweighted', 'acc_kmedioid_k', 'acc_kmediod_kplus', 'acc_svc', 'acc_rbf'])
